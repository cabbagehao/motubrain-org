...
import { indexName, tableName } from './naming';

const { pgTableWithSchema } = require('drizzle-orm/pg-core/table') as {
  pgTableWithSchema: (
    name: string,
    columns: unknown,
    extraConfig: unknown,
...
// Drizzle forbids pgSchema('public'), but production pooler connections can have an
// empty search_path. We therefore generate schema-qualified tables for public too.
const customSchema =
  schemaName && schemaName !== 'public' ? pgSchema(schemaName) : null;
const rawTable: PgTableFn = customSchema
  ? (customSchema.table.bind(customSchema) as unknown as typeof drizzlePgTable)
...
const table: PgTableFn = ((name: string, columns: any, extraConfig: any) =>
  rawTable(tableName(name) as any, columns, extraConfig)) as PgTableFn;
const index = (name: string) => drizzleIndex(indexName(name));
