...
import { trackGaEvent, trackSelectContent } from '@/shared/lib/ga4';
...
export function SocialProviders({
  ...
  const handleAuthCallback = useCallback(() => {
    if (activeProviderRef.current) {
      trackGaEvent('login', {
        method: activeProviderRef.current,
        page_type: 'sign_in',
      });
    }
    cleanupPopup();
  ...
  const handleSignIn = async ({ provider }: { provider: string }) => {
    ...
    trackSelectContent({
      contentId: `auth_social_${provider}`,
      contentType: 'auth_social',
      section: 'sign_in',
      entryPoint: 'sign_modal',
    });
