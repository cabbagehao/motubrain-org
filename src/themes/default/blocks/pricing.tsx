...
import { trackBeginCheckout, trackSelectItem } from '@/shared/lib/ga4';
...
export function Pricing({
  ...
  const handlePayment = async (item: PricingItem) => {
    ...
    trackSelectItem({
      item: displayedItem,
      itemListName: pricing.title || 'pricing',
      section: pricing.id || group || 'pricing',
    });
  ...
  const handleCheckout = async (
    ...
    try {
      ...
      trackBeginCheckout({
        item,
        provider: paymentProvider || configs.default_payment_provider,
        section: pricing.id || group || 'pricing',
        itemListName: pricing.title || 'pricing',
      });
